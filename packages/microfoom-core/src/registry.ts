...
type ExposureTier = "silent" | "announcement" | "tool";
...
function classMetaForCtor(ctor: object): ClassMeta {
...
function methodMetaFor(classMeta: ClassMeta, name: string): MethodMeta {
...
function readClassMeta(instance: object): ClassMeta | undefined {
  return store.get(instance.constructor);
}

/** The exposed methods of an instance, by dispatch name. */
function exposedMethods(instance: object): Map<string, ExposeMeta> {
  ...
  for (const method of meta.methods.values()) {
    if (method.expose !== undefined) {
      result.set(method.expose.dispatchName, method.expose);
    }
  }
  return result;
}

export type { ExposeMeta, ExposureTier };
export { classMetaForCtor, exposedMethods, methodMetaFor, readClassMeta };
