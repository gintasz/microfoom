...
describe("microfoom config", () => {
  it("defaults a program name to its filename stem (audit.ts → audit)", () => {
    const config = parseConfig({ programs: [{ type: "command", path: "progs/audit.ts" }] }, "/cfg");
  ...
  it("resolves model default + per-entry override", () => {
    const config = parseConfig(
      { model: "root-model", programs: [{ type: "command", path: "a.ts", model: "entry-model" }] },
      "/cfg",
    );
  ...
  it("requires type to be command or tool", () => {
  ...
  it("requires a description on a tool entry", () => {
    expect(() => parseConfig({ programs: [{ type: "tool", path: "a.ts" }] }, "/cfg")).toThrow(
      FoomtimeConfigError,
    );
  });
  ...
  it("rejects duplicate names within a namespace", () => {
    expect(() =>
      parseConfig(
        {
          programs: [
            { type: "command", path: "a.ts", name: "x" },
            { type: "command", path: "b.ts", name: "x" },
          ],
        },
        "/cfg",
      ),
    ...
    const config = parseConfig(
      {
        programs: [
          { type: "command", path: "a.ts", name: "x" },
          { type: "tool", path: "a.ts", name: "x", description: "does x" },
        ],
      },
      "/cfg",
    );
