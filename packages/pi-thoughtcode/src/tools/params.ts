...
import { parseVibeFunctionParams, type ParsedParam } from "thoughtcode-core";
import { isParsableReturnType, validateValue } from "./return-type.js";

export type ResolvedParams =
  | { status: "ok"; params: ParsedParam[] }
  | { status: "invalid"; errors: string[] }
  | { status: "unreadable" };

/** Read the program file and resolve a VIBEFUNCTION's parameter declarations (types validated). */
export async function resolveParams(
  ...
  return { status: "ok", params: parsed.params };
}
