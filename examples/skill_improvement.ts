...
export default class SkillImprovement extends Program(z.void()) {
  async main(): Promise<void> {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      const batch_size = 3;
      const prompts = await this.createPrdPrompts(i, batch_size);
      await Promise.all(
        prompts.map(
  ...
  async createPrdPrompts(iteration: number, batch_size: number): Promise<string[]> {
    return await this.agent.with({ label: "create-prd-prompts" }).value(z.array(z.string()))`
        ...
        make_prompt(i):
            ...
            prompt += <ask it to use ./SKILL-[${iteration}].md file by providing path to it>
            prompt += <ask it to dump PRD to ./PRD-[${iteration}]-[i].md file>
  ...
  async improveSkill(iteration: number, batch_size: number): Promise<void> {
    return await this.agent.value(z.any())`
