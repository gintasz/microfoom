...
╚══════════════════════════════════════════════════════════════════════╝
```
- **B1.** No secrets in source, logs, or error messages. *(review — no secret-scanning gate is wired yet; add one (e.g. gitleaks) to make this mechanical)*
...
- **A2. Unit of modularity** `[consistency]` — The package. Each package exposes exactly one curated public surface (its barrel) — curated and intentional, not an incidental re-export of everything. Internals are reached only through that surface; each package's public surface is captured as a reviewable public-surface report (the reporter is named in ADR-0001) so surface changes are diffable, not silent. *(tool: linter + import-graph checker + public-surface reporter)*
- **A3. Dependency direction** `[tradeoff]` — Dependencies flow one way: from the more specific to the more generic (a harness may import the generic core; the generic core must never import a harness or anything harness-specific). The direction is partly enumerated per package in the import-graph config, so this principle is what tells you which rule to add for a new package. (No-cycles is a separate, general, fully-mechanical rule — not restated here.) *(tool: import-graph checker)*
- **A4. Boundary access** `[tradeoff]` — Cross-package access via the public surface only; the deep-import ban is fully enforced by a general import-graph rule (kept as an ID anchor, not an English prescription). *(tool: import-graph checker)*
...
- **N1. Files & directories** `[consistency]` — One cohesive module per file (not one-export-per-file). File and directory name casing is linter-enforced and governed via L1; not restated here. *(review)*
- **N2. Identifier casing** — *retired.* Identifier casing is authoritatively the linter's (config governed via L1/ADR-0001); restating the casing values here was a second source of truth. ID retired, not reused (P7).
...
- **N4. Abbreviations** `[consistency]` — Banned except established domain terms (N5). No single-letter names except loop indices and well-scoped lambdas. *(review)*
- **N5. Ubiquitous language** `[tradeoff]` — One term per concept across code and prompts. Spell-checking is correction-based (flags known misspellings, not unknown words), so there is no domain-word allowlist or glossary to curate. Term consistency (non-canonical synonyms, naming drift) is a review concern, not auto-enforced. *(tool: spell checker + review)*
...
- **L1. Versions & toolchain** `[consistency]` — One repo-wide toolchain, pinned in the lockfile and `package.json`: a single global choice of language, build system, package manager, schema contract, formatter, linters, import-graph and custom code-pattern enforcers, public-surface reporter, test runner, property-based-testing engine, and monorepo-hygiene gates (dead-code, duplication, version-policy, spell, commit, git-hook). The language is strict, statically-typed TypeScript with a type-authoritative project-reference build. **The concrete members — and every compiler flag — live in their config files and are catalogued in ADR-0001, not enumerated here** — naming a specific tool, package, or flag is an instance decision, not a principle (P1/P2), that would churn this document on every swap and create a second source of truth. (A member with no npm build is pinned in CI, not the lockfile — see ADR-0001.) A same-role swap is a C3 catalogue edit; adding or removing a gate is a P6 amendment. *(tool: build/CI)*
...
- **L3. Banned constructs** `[consistency]` — Stringly-typed domain failures and per-function ad-hoc `{ ok }`/`Result` shapes are forbidden in place of the one error taxonomy (T6/F7) or a discriminated union (T3). This is the one banned-construct rule that is genuinely review-enforced — every other ban (`export *`, deep imports, `any`, non-null `!`, module-global mutable state) is fully mechanical and lives only in its linter, not in this document. *(review)*

## T. Types & Data Modeling
*Tier: Core*

- **T1. Type strictness** `[tradeoff]` — Maximum static strictness; the concrete compiler flags (and the `any` ban) are tool-enforced and live in tsconfig/linter config, not restated here. The stdlib is made sound with a sound-stdlib library (`JSON.parse`→`unknown`, checked indexing). Utility types come from a canonical utility-types library, not bespoke reinvention (parsimony). Unsafe casts are confined to a single isolated vendor-wrapper module (DEP2); a cast anywhere else must surface, not be silenced — this confinement is the prescriptive content here, since no rule limits *where* casts appear. *(tool: type checker + review)*
...
- **C3. ADRs** `[tradeoff]` — An ADR records a decision that is *costly to reverse* or *genuinely divergent* (P2): control-tool naming, the config option/cascade set, exposure-tier mechanics, harness adapters, ratified amendments (P6). A reversible same-role swap — one tool for another, interface unchanged — is a catalogue edit in ADR-0001, not a new ADR. *(review)*
...
- **DEP3. Version policy** `[consistency]` — Upgrades are deliberate, not floating. The concrete policy (exact pins, one version per dependency, workspace protocol) is fully enforced in the version-policy checker's config — not restated here. *(tool: version-policy checker + build/CI)*
